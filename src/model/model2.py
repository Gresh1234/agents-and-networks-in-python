...
from src.space.road_network2 import CampusWalkway
...
class AgentsAndNetworks(mesa.Model):
    ...
    def __init__(
        ...
        self._load_buildings_from_file(osm_object, crs=model_crs)
        self._load_road_vertices_from_file(osm_object, crs=model_crs, campus="scl")
        ...
        if show_driveway:
            self._load_driveway_from_file(osm_object, crs=model_crs)
        if show_lakes_and_rivers:
            self._load_lakes_and_rivers_from_file(osm_object, crs=model_crs)
    ...
    def _load_buildings_from_file(self, osm_object: OSM, crs: str) -> None:

        buildings_df = osm_object.get_buildings()
    ...
    def _load_road_vertices_from_file(
        self, osm_object: OSM, crs: str, campus: str
    ) -> None:
        walkway_df = (
            osm_object.get_network("walking")
            .set_crs(self.data_crs, allow_override=True)
            .to_crs(crs)
            .explode(ignore_index = True)
            .reset_index()
        )
        self.walkway = CampusWalkway(campus=campus, osm_object=osm_object)
    ...
    def _load_driveway_from_file(self, osm_object: OSM, crs: str) -> None:
        driveway_df = (
            osm_object.get_network("driving")
            .set_index("id")
            .set_crs(self.data_crs, allow_override=True)
            .to_crs(crs)
            .explode(ignore_index = True)
